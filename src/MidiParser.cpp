...
MidiParser::CV_id_abcd MidiParser::ccToCVid_abcd(uint8_t cc)
{
    ...
    {
        case 0:
            return CV_id_abcd::bank_00;    // For "strong typing" we use typed enums, but still cast to normal numeric values, because we need them as indexes
        case 32:
            return CV_id_abcd::sbnk_32;
        case 1:
            return CV_id_abcd::mw_01;
        case 2:
            return CV_id_abcd::bc_02;
        case 71:
            return CV_id_abcd::res_71;
        case 72:
            return CV_id_abcd::rel_72;
        case 73:
            return CV_id_abcd::atk_73;
        case 74:
            return CV_id_abcd::cut_74;
        case 75:
            return CV_id_abcd::sc6_75;
        case 76:
            return CV_id_abcd::sc7_76;
        case 77:
            return CV_id_abcd::sc8_77;
        case 78:
            return CV_id_abcd::sc9_78;
        case 123:           // All notes off, not mappable, but used to directly reset current triggers to prevent hanging notes...
            ...
            return CV_id_abcd::abcd_cc_invalid;   // Unexpected value, so we know the incoming CC can't be mapped to values known for the Voices A-D
        default:
            return CV_id_abcd::abcd_cc_invalid;   // Unexpected value, so we know the incoming CC can't be mapped to values known for the Voices A-D
...
MidiParser::CV_id_glob MidiParser::ccToCVid_glob(uint8_t* msg)
{
    switch(*(msg+1))     // Voices A-D: Convert incoming CC-numbers to valid enum of processed CCs or 'none' which effectively is 0 and unused!
    {
        case 0:                     // Bankchange: Set global bank, this may lead to select a new plugin on Slot 0 when triggered via programchange
            glob_bank = *(msg+2);
            return CV_id_glob::glob_cc_invalid; // Unexpected value, so we know the incoming CC can't be mapped to values known for the globals
        case 1:
            return CV_id_glob::g_mw_01;         // For "strong typing" we use typed enums, but still cast to normal numeric values, because we need them as indexes
        case 2:
            return CV_id_glob::g_bc_02;
        case 4:
            return CV_id_glob::g_foot_04;
        case 6:
            return CV_id_glob::g_data_06;
        case 7:
            return CV_id_glob::g_vol_07;
        case 8:
            return CV_id_glob::g_bal_08;
        case 10:
            return CV_id_glob::g_pan_10;
        case 11:
            return CV_id_glob::g_expr_11;
        case 12:
            return CV_id_glob::g_fx1_12;
        case 13:
            return CV_id_glob::g_fx2_13;
        case 32:                        // Subbankchange: Set global sub-bank if received via bankchange, this may lead to select a new plugin on Slot 1 when triggered via programchange
            ...
            return CV_id_glob::glob_cc_invalid;     // Unexpected value, so we know the incoming CC can't be mapped to values known for the globals
        case 64:
            return CV_id_glob::g_sustp_64;
        case 65:
            return CV_id_glob::g_portp_65;
        case 66:
            return CV_id_glob::g_sstnp_66;
        case 67:
            return CV_id_glob::g_softp_67;
        case 69:
            return CV_id_glob::g_holdp_69;
...
MidiParser::TRIG_id_abcd MidiParser::ccToTrigId_abcd(uint8_t cc)
{
    ...
    {
        case 75:
            return TRIG_id_abcd::t_sc6_75;      // For "strong typing" we use typed enums, but still cast to normal numeric values, because we need them as indexes
        case 76:
            return TRIG_id_abcd::t_sc7_76;
        case 77:
            return TRIG_id_abcd::t_sc8_77;
        case 79:
            return TRIG_id_abcd::t_sc9_79;
        default:
            return TRIG_id_abcd::t_abcd_cc_invalid;   // Unexpected value, so we know the incoming CC can't be mapped to values known for the TBD BBA...
    }
}
...
MidiParser::TRIG_id_glob MidiParser::ccToTrigId_glob(uint8_t cc)
{
    ...
    {
        case 12:
            return TRIG_id_glob::t_g_fx1_12;  // For "strong typing" we use typed enums, but still cast to normal numeric values, because we need them as indexes
        case 13:
            return TRIG_id_glob::t_g_fx2_13;
        case 64:
            return TRIG_id_glob::t_g_sustp_64;
        case 65:
            return TRIG_id_glob::t_g_portp_65;
        case 66:
            return TRIG_id_glob::t_g_sostp_66;
        case 67:
            return TRIG_id_glob::t_g_softp_67;
        case 69:
            return TRIG_id_glob::t_g_holdp_69;
        default:
            return TRIG_id_glob::t_glob_cc_invalid;   // Unexpected value, so we know the incoming CC can't be mapped to values known for the TBD BBA...
    }
}
...
MidiParser::Control_element_cv_id MidiParser::noteToCVid_perc(uint8_t drum_note)
{
    ...
    {
        case 36:
            return Control_element_cv_id::A_SC6_75;
        case 37:
            return Control_element_cv_id::A_SC7_76;
        case 38:
            return Control_element_cv_id::A_SC8_77;
        case 39:
            return Control_element_cv_id::A_SC9_78;
        case 40:
            return Control_element_cv_id::B_SC6_75;
        case 41:
            return Control_element_cv_id::B_SC7_76;
        case 42:
            return Control_element_cv_id::B_SC8_77;
        case 43:
            return Control_element_cv_id::B_SC9_78;
        case 44:
            return Control_element_cv_id::C_SC6_75;
        case 45:
            return Control_element_cv_id::C_SC7_76;
        case 46:
            return Control_element_cv_id::C_SC8_77;
        case 47:
            return Control_element_cv_id::C_SC9_78;
        case 48:
            return Control_element_cv_id::D_SC6_75;
        case 49:
            return Control_element_cv_id::D_SC7_76;
        case 50:
            return Control_element_cv_id::D_SC8_77;
        case 51:
            return Control_element_cv_id::D_SC9_78;
        default:
            return Control_element_cv_id::A_NOTE; // Unexpected result, equivalent to 0!
    }
}
...
void MidiParser::allNotesOff()                // Typically to be triggered by all notes off message (Continouus Controller)
{
    ...
    for( int polyChannels=1; polyChannels<5; polyChannels++)           // Reset MPT/Mono- Duo- and/or Polyphonic notes
    {
        ...
        trig_entry = trig_distributor[polyChannels][(uint8_t)TRIG_id_abcd::t_note];  // Retrieve MIDI-channel (internally 0-15) by masking the MIDI status-byte, We cast the enum retrieved in order to use it as a regular array-index
...
void MidiParser::handleNoteOff(uint8_t*  msg)
{
    ...
    if( IS_PERCUSSION(channel) )                         // ### MB20240911: Now MIDI-channels 10-13 are available for per-note triggers / typically percussions
    {
        ...
        if( (drum_trigger >= PERCUSSION_NOTE_LOW) && (drum_trigger <= PERCUSSION_NOTE_HIGH) )   // Check if we have percussion-notes within range on our drum-channel
        {
            ...
            drum_trigger = drum_trigger-PERCUSSION_NOTE_LOW + ((uint8_t)Control_element_trig_id::T_A_SC6_75);            // We have a common trigger to be set, as used with CCs 75-78 on Voices A-D (for details see specification)
    ...
    if( IS_MONOPHONIC(channel) )
    {
        ...
        monophonic_keys_down = std::max(0, monophonic_keys_down-1 ); // Reduce number of pressed notes, make sure to not become negative by accident
        ...
        if( monophonic_keys_down )  // Still notes pressed?
        {
            for( int i=0; i<sizeof(midi_note_pressed[vmode_mono]); i++)
            {
                if(midi_note_pressed[vmode_mono][i])              // Entry for Legato found?
                {
                    ...
                    midi_cvs[(uint8_t)Control_element_cv_id::A_NOTE] = std::min(1.f, (msg[1]-MIDI_NOTE_0V)*SCALEVALUE_MIDI_NOTE_TO_CV);    // Set CV at given offset as just retrieved, rescale according to CV from CCs
                    midi_cvs[(uint8_t)Control_element_cv_id::A_VELO] = msg[2] * SCALEVALUE_MIDI_CTRLS_TO_CV;      // Set CV at given offset as just retrieved, rescale according to CV from CCs
            ...
            for( int legato_note_pitch=0; legato_note_pitch<sizeof(midi_note_pressed[voice_mode]); legato_note_pitch++)
            {
                if(midi_note_pressed[voice_mode][legato_note_pitch])              // Entry for Legato found?
                {
                    ...
                    if(!ignore_playing_note)                // We found a note in the list that is not identical with a already playing for any voice
                    {
                        ...
                        cv_entry = cv_distributor[channel][(uint8_t)CV_id_abcd::note];  // Retrieve MIDI-channel (internally 1-4) by masking the MIDI status-byte, We cast the enum retrieved in order to use it as a regular array-index
                        midi_cvs[(uint8_t)cv_entry.element_cv_id] = std::min(1.f, (msg[1]-MIDI_NOTE_0V)*SCALEVALUE_MIDI_NOTE_TO_CV);    // Set CV at given offset as just retrieved, rescale according to CV from CCs

                        cv_entry = cv_distributor[channel][(uint8_t)CV_id_abcd::velocity];  // Retrieve MIDI-channel (internally 1-4) by masking the MIDI status-byte, We cast the enum retrieved in order to use it as a regular array-index
    ...
    trig_entry = trig_distributor[channel][(uint8_t)TRIG_id_abcd::t_note];         // Retrieve MIDI-channel (internally 0-15) by masking the MIDI status-byte, We cast the enum retrieved in order to use it as a regular array-index
...
void MidiParser::controlChange(uint8_t* msg)
{
    ...
    if( channel != MIDI_GLOBAL_CHANNEL ) // Voices A-D
    {
        ...
        if(abcdCVid != CV_id_abcd::abcd_cc_invalid) // Valid CC for CV found?
        {
        ...
        if( abcdTRIGid != TRIG_id_abcd::t_abcd_cc_invalid )   // Valid CC for Trigger found?
        {
    ...
    else    // Global channel (internally 0)
    {
        ...
        if( globCVid != CV_id_glob::glob_cc_invalid)    // Valid CC for CV found?
        {
        ...
        if( globTRIGid != TRIG_id_glob::t_glob_cc_invalid )   // Valic CC for Trigger found?
        {
...
void MidiParser::channelPressure(uint8_t* msg)
{
    ...
    {
        cv_entry = cv_distributor[channel][(uint8_t)CV_id_glob::g_at];          // Retrieve MIDI-channel (internally 05) by masking the MIDI status-byte, We cast the associated enum in order to use it as a regular array-index
        trig_entry = trig_distributor[channel][(uint8_t)TRIG_id_glob::t_g_at];    // Retrieve MIDI-channel (internally 05) by masking the MIDI status-byte, We cast the enum retrieved in order to use it as a regular array-index
    }
    else            // Voices A-D
    {
        channel = (channel-1)%4 +1;         // We 'map down' MIDI-channels above 2-5 to channels, 2-5 accordingly! So via "voice-stealing", Channel 6 will be stored as 2 again and so on
        cv_entry = cv_distributor[channel][(uint8_t)CV_id_abcd::aftertouch];  // Retrieve MIDI-channel (internally 05) by masking the MIDI status-byte, We cast the associated enum in order to use it as a regular array-index
        trig_entry = trig_distributor[channel][(uint8_t)TRIG_id_abcd::t_aftertouch];  // Retrieve MIDI-channel (internally 05) by masking the MIDI status-byte, We cast the enum retrieved in order to use it as a regular array-index
    }
...
void MidiParser::programChange(uint8_t* msg)
{
    ...
    cv_entry = cv_distributor[channel][(uint8_t)CV_id_abcd::progchange];  // Retrieve MIDI-channel (internally 0-15) by masking the MIDI status-byte, We cast the associated enum in order to use it as a regular array-index
    midi_cvs[(uint8_t)cv_entry.element_cv_id] = msg[1] * SCALEVALUE_MIDI_CTRLS_TO_CV;      // Set CV at given offset as just retrieved. (We get rid of the first "none"-entry to calculate the CV-index). Rescale according to 0.-1.f according 0-5V CV from CCs

    trig_entry = trig_distributor[channel][(uint8_t)TRIG_id_abcd::t_progchange];  // Retrieve MIDI-channel (internally 0-15) by masking the MIDI status-byte, We cast the enum retrieved in order to use it as a regular array-index
    midi_triggers[((uint8_t)trig_entry.element_trig_id)] = (msg[1] < 64);  // Set Trigger according to value from Continuous controller (We get rid of the first "none"-entry to calculate the CV-index)Set Trig to 1 if off, 0 if on at given offset as just retrieved
}
...
void MidiParser::pitchBend(uint8_t* msg)
{
    ...
    {
        pitchBend_val = std::max(-8191, pitchBend_val_raw-8192);
        midi_cvs[(uint8_t)Control_element_cv_id::G_PB] = pitchBend_val * SCALEVALUE_MIDI_PITCHBEND_TO_CV;      // Set CV at given offset as just retrieved. (We get rid of the first "none"-entry to calculate the CV-index). Rescale according to 0.-1.f according 0-5V CV from CCs
        pitchBend_val = lut_logarithmic_10bit[pitchBend_val_raw>>4];    // Use 10bit of 14 bit value for logarithmic scaling
        midi_cvs[(uint8_t)Control_element_cv_id::G_PB_LG] = pitchBend_val * SCALEVALUE_MIDI_PITCHBEND_TO_CV_10BIT;      // Set CV at given offset as just retrieved. (We get rid of the first "none"-entry to calculate the CV-index). Rescale according to 0.-1.f according 0-5V CV from CCs
    }
    else    // Voices A-D - Smallest pitch bend is 0, largest is 16383, 8192 means no Pitchbend, msg[1] is LSB (zero if unused), msg[2] is MSB ---
    {
        ...
        cv_entry = cv_distributor[channel][(uint8_t)CV_id_abcd::pitchbend];         // Retrieve MIDI-channel (internally 1-4) by masking the MIDI status-byte, We cast the enum retrieved in order to use it as a regular array-index
        pitchBend_val = std::max(-8191,pitchBend_val_raw-8192);
        ...
        cv_entry = cv_distributor[channel][(uint8_t)CV_id_abcd::pitchbend_log];     // Retrieve MIDI-channel (internally 1-4) by masking the MIDI status-byte, We cast the enum retrieved in order to use it as a regular array-index
        pitchBend_val = lut_logarithmic_10bit[pitchBend_val_raw>>4];    // Use 10bit of 14 bit value for logarithmic scaling
...
void MidiParser::noteOn(uint8_t* msg)
{
    ...
    if(new_pitch_and_velo_for_note)     // This may have been inactivated via Mono-Mode
    {
        ...
        trig_entry = trig_distributor[channel][(uint8_t)TRIG_id_abcd::t_velocity];  // Retrieve MIDI-channel (internally 0-15) by masking the MIDI status-byte, We cast the enum retrieved in order to use it as a regular array-index
        midi_triggers[(uint8_t)trig_entry.element_trig_id] = (msg[2] < 64);  // Set Trigger according to value from Continuous controller (We get rid of the first "none"-entry to calculate the CV-index)Set Trig to 1 if off, 0 if on at given offset as just retrieved, rescale according to CV from CCs
    }
}
