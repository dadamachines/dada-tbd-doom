...
void I_InitSound(boolean use_sfx_prefix)
{
    ...
    if (!nosound && !screensaver_mode)
    {
...

        {
            ...
            debug_show_hex2(0xA0000001); // A.1: before InitSfxModule
        }
        if (!nosfx)
        {
        ...
        {
            extern void debug_show_hex2(uint32_t);
            debug_show_hex2(0xA0000002); // A.2: before InitMusicModule
        }
        if (!nomusic)
        {
            InitMusicModule();
        ...
        {
            extern void debug_show_hex2(uint32_t);
            debug_show_hex2(0xA0000003); // A.3: after InitMusicModule
        }
