...
#else
#if PICO_ON_DEVICE
        // Use __end__ (end of BSS) as zone base.
        // Note: __HeapLimit means "top of RAM" in earlephilhower framework,
        // not "end of BSS" as in vanilla pico-sdk, so we must use __end__.
        extern char __end__;
        zonemem = (uint8_t *)(((uintptr_t)&__end__)&~3);
        *size = ((uint8_t *)SRAM4_BASE) - zonemem;
        // Diagnostic: show zone base and size on OLED
        {
            extern void debug_show_hex(uint32_t);
            extern void debug_show_hex2(uint32_t);
            debug_show_hex((uint32_t)(uintptr_t)zonemem);
            debug_show_hex2((uint32_t)*size);
        }
#else
