...
{
    {
        extern void debug_show_hex(uint32_t);
        static int frame_count = 0;
        if (frame_count == 0) {
            debug_show_hex(0xEE000001); // first D_RunFrame entry
        }
        frame_count++;
    }
#if !DOOM_TINY
    ...
    I_StartFrame ();
    {
        extern void debug_show_hex(uint32_t);
        static int sf1 = 0;
        if (sf1 == 0) debug_show_hex(0xEE000002); // after I_StartFrame
        sf1++;
    }
    ...
    TryRunTics (); // will run at least one tic
    {
        extern void debug_show_hex(uint32_t);
        static int sf2 = 0;
        if (sf2 == 0) debug_show_hex(0xEE000003); // after TryRunTics
        sf2++;
    }

    {
        ...
        static int sf2b = 0;
        if (sf2b == 0) {
            // Show consoleplayer and mo pointer for diagnosis 
            debug_show_hex((uint32_t)(uintptr_t)players[consoleplayer].mo);
        }
        sf2b++;
    }
    S_UpdateSounds (players[consoleplayer].mo);// move positional sounds
    {
        extern void debug_show_hex(uint32_t);
        static int sf3 = 0;
        if (sf3 == 0) debug_show_hex(0xEE000004); // after S_UpdateSounds
        sf3++;
    }
...
void D_DoomLoop (void)
{
    ...
    {
        extern void debug_show_hex(uint32_t);
        debug_show_hex(0xDD000001); // after I_InitGraphics
    }
#if USB_SUPPORT
    ...
    {
        extern void debug_show_hex(uint32_t);
        debug_show_hex(0xDD000002); // after EnableLoadingDisk
    }

    TryRunTics();
    {
        extern void debug_show_hex(uint32_t);
        debug_show_hex(0xDD000003); // after TryRunTics
    }
    ...
    {
        extern void debug_show_hex(uint32_t);
        debug_show_hex(0xDD000004); // after R_ExecuteSetViewSize
    }

    D_StartGameLoop();
    {
        extern void debug_show_hex(uint32_t);
        debug_show_hex(0xDD000005); // after D_StartGameLoop, entering main loop
    }
...
//    }
    ...
    debug_show_stage(5); // Stage 5: Z_Init
    ...
    debug_show_stage(6); // Stage 6: WAD init
...
//    W_PrintDirectory();
    ...
    debug_show_stage(7); // Stage 7: I_Init (sound/timer)
    ...
    {
        extern void debug_show_hex(uint32_t);
        debug_show_hex(0x70000001); // 7.1: before I_InitSound
    }
    I_InitSound(true);
    {
        extern void debug_show_hex(uint32_t);
        debug_show_hex(0x70000002); // 7.2: after I_InitSound
    }
    I_InitMusic();
    {
        extern void debug_show_hex(uint32_t);
        debug_show_hex(0x70000003); // 7.3: after I_InitMusic
    }
    ...
    {
        extern void debug_show_hex(uint32_t);
        debug_show_hex(0x70000004); // 7.4: before M_Init
    }
    ...
    {
        extern void debug_show_hex(uint32_t);
        debug_show_hex(0x70000005); // 7.5: after M_Init
    }
    debug_show_stage(8); // Stage 8: R_Init
    DEH_printf("R_Init: Init DOOM refresh daemon - ");
    R_Init ();

    {
        extern void debug_show_hex(uint32_t);
        debug_show_hex(0x80100001); // after R_Init, before P_Init
    }
    ...
    {
        extern void debug_show_hex(uint32_t);
        debug_show_hex(0x80100002); // after P_Init, before S_Init
    }
    ...
    {
        extern void debug_show_hex(uint32_t);
        debug_show_hex(0x80100003); // after S_Init
    }

    {
        extern void debug_show_hex(uint32_t);
        debug_show_hex(0xCC000001); // before D_CheckNetGame
    }
#if !USE_PICO_NET
    ...
    {
        extern void debug_show_hex(uint32_t);
        debug_show_hex(0xCC000009); // after D_CheckNetGame
    }

    PrintGameVersion();
    {
        extern void debug_show_hex(uint32_t);
        debug_show_hex(0xCC00000A); // after PrintGameVersion
    }
    ...
    {
        extern void debug_show_hex(uint32_t);
        debug_show_hex(0xCC000004); // after HU_Init
    }
    ...
    {
        extern void debug_show_hex(uint32_t);
        debug_show_hex(0xCC000005); // after ST_Init
    }
    ...
    {
        extern void debug_show_hex(uint32_t);
        debug_show_hex(0x80100006); // before gameaction check
    }
    ...
    {
        extern void debug_show_hex(uint32_t);
        debug_show_hex(0x80100007); // entering D_DoomLoop
    }
