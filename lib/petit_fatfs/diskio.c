...
void disk_deinitialize(void){
  spi_deinit(SD_SPICH);
  gpio_deinit(SD_SPI_MISO);
  gpio_deinit(SD_SPI_MOSI);
  gpio_deinit(SD_SPI_SCK);
  gpio_deinit(SD_SPI_CS);
  // power-cycle sd
  gpio_put(SD_RESET_PIN, 1);
  sleep_us(100000); /* 100ms */
  gpio_put(SD_RESET_PIN, 0);
  sleep_us(100000); /* 100ms */
  gpio_deinit(SD_RESET_PIN);
  Stat = STA_NOINIT;
  CardType = 0;
}
...
DSTATUS disk_initialize(void)
{
  ...
  // power-cycle sd
  gpio_set_dir(SD_RESET_PIN, GPIO_OUT);
  gpio_put(SD_RESET_PIN, 1);
  sleep_us(100000); /* 100ms */
  gpio_put(SD_RESET_PIN, 0);
  sleep_us(100000); /* 100ms */
